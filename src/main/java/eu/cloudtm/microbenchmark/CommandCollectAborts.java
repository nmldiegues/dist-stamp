...
public abstract class CommandCollectAborts<T> {
    ...
    public CommandCollectAborts() {
	this.readOnly = false;
    }

    public CommandCollectAborts(boolean readOnly) {
	...
	while (!txFinished) {
	    try {
		aborts++;
		Micro.txManager.begin();
		if (!readOnly) {
		    Micro.cache.markAsWriteTransaction();
		}

		result = runTx();

		Micro.txManager.commit();
